...
public class MmpldData {
	...
	public MmpldData(final Vector3f boxMin, final Vector3f boxMax, final List<float[]> dataFrames, final Vector4f globalRgba, final int maxParticlesPerFrame, int numberOfDataFrames) {
		this.boxMin = boxMin;
		this.boxMax = boxMax;
	...
	public static MmpldData parseFrom(final File mmpldFile) throws DataFormatException, IOException, InterruptedException {
		...
		if (vertexDataType!=VertexDataType.NONE) {
			...
			float[] particles = new float[pCount*3];	//TODO use buffer directly ?
		...
		mmpldData = new MmpldData(boxMin, boxMax, dataFrames, globalRgba, maxParticlesPerFrame, Math.min(numberOfDataFrames, MAX_FRAMES_READ));
